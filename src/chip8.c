...
static void chip8_exec_extended_eight(struct chip8 *chip8, unsigned short opcode)
{
    ...
    // 8xy1 - OR vx, vy. BITWISE VX OR VY STORE IN VX
    case 0x01:
        chip8->registers.V[x] = chip8->registers.V[x] | chip8->registers.V[y];
        break;

    // 8xy2 - AND vx, vy. BITWISE VX OR VY STORE IN VX
    case 0x02:
        chip8->registers.V[x] = chip8->registers.V[x] & chip8->registers.V[y];
        break;

        // 8xy3- EXCLUSIVE OR vx, vy. BITWISE VX EXCLUSIVE OR VY STORE IN VX
    case 0x03:
        chip8->registers.V[x] = chip8->registers.V[x] ^ chip8->registers.V[y];
        break;

    // 8xy4- set vx ADD vx, vy. set vf = carry
    case 0x04:
        ...
        // 8xy4- set vx sub vx, vy. set vf = no borrow
    case 0x05:
        chip8->registers.V[0x0f] = false;
...
static void chip8_exec_extended_F(struct chip8 *chip8, unsigned short opcode)
{
    ...
    case 0x0A:
    {
        char key_pressed = chip8_wait_for_key_press(chip8);
...
static void chip8_exec_extended(struct chip8 *chip8, unsigned short opcode)
{
    ...
    {
    case 0x1000:
        // JP addr, 1nnn jmp to location nnn's
        chip8->registers.PC = nnn;
        break;

    case 0x2000:
        // CALL addr, 2nnn call subroutine to location nnn
    ...
    case 0xB000:
            chip8->registers.I = nnn + chip8->registers.V[0x00];
        break;
    ...
    case 0xD000:
    {
        ...
        chip8->registers.V[0x0f] = chip8_screen_draw_sprite(&chip8->screen, chip8->registers.V[x], chip8->registers.V[y],
                                     sprite, n);
    }
    break;
